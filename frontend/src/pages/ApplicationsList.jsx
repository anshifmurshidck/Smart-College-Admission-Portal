...
export default function ApplicationsList() {
  ...
  const loadApplications = async () => {
    ...
    try {
      ...
      const response = await adminFetch(`${API_BASE}/admin/applications?${params.toString()}`, {
  ...
  const viewApplicationDetails = async (appId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await adminFetch(`${API_BASE}/admin/applications/${appId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleStatusSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await adminFetch(`${API_BASE}/admin/applications/${selectedApp}/status`, {
        method: 'POST',
        headers: { 
  ...
  const updateApplicationStatus = async (appId, newStatus) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await adminFetch(`${API_BASE}/admin/applications/${appId}/status`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
