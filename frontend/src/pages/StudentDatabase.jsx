...
export default function StudentDatabase() {
  ...
  const loadStudents = async () => {
    ...
    try {
      ...
      const response = await adminFetch(`${API_BASE}/admin/students?${params.toString()}`, {
  ...
  const viewStudent = async (studentId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await adminFetch(`${API_BASE}/admin/students/${studentId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await adminFetch(`${API_BASE}/admin/students/${selectedStudent}`, {
        method: 'PUT',
        headers: { 
  ...
  const handleAddStudent = async (e) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await adminFetch(`${API_BASE}/admin/students/add`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
