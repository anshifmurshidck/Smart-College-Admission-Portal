  ...
  const handleSendMessage = (e) => {
    ...
    setTimeout(async () => {
      ...
      if (matchedAppId || (isWaitingForAppId && userText.toUpperCase().startsWith('APP-'))) {
        ...
        // Keyword parsing
        if (lowerText.includes('apply') || lowerText.includes('admission') || lowerText.includes('register') || lowerText.includes('process') || lowerText.includes('enroll')) {
          botResponseText = "TMEC Admissions are fully digital. You can submit your form in under 10 minutes at the [Apply Now Page](/apply).\n\nSteps:\n1. Fill details\n2. Upload Marksheets & ID proof\n3. Wait for our committee review (usually 24 hours)\n4. Receive your unique TMEC Student ID upon approval!";
        } else if (lowerText.includes('branch') || lowerText.includes('department') || lowerText.includes('course') || lowerText.includes('cse') || lowerText.includes('aiml') || lowerText.includes('ece') || lowerText.includes('mech') || lowerText.includes('civil')) {
          botResponseText = "TMEC offers 5 specialized B.Tech programs:\n\n* **Computer Science (CSE)**\n* **Artificial Intelligence (AIML)**\n* **Electronics (ECE)**\n* **Mechanical (ME)**\n* **Civil Engineering (CE)**\n\nLearn more details about curriculum, HODs, and labs on our [Academic Departments Page](/departments).";
        } else if (lowerText.includes('fee') || lowerText.includes('cost') || lowerText.includes('price') || lowerText.includes('scholarship') || lowerText.includes('criteria') || lowerText.includes('eligibility')) {
          botResponseText = "**Admissions Eligibility:**\n* B.Tech Programs: Minimum 60% aggregate in 12th (Physics, Chemistry, Maths).\n\n**Fee Structure (per semester):**\n* CSE / AIML: ₹95,000\n* ECE / ME / CE: ₹80,000\n\n*Scholarships: Students with >90% in 12th Boards get 25% tuition fee waiver!*";
        } else if (lowerText.includes('track') || lowerText.includes('status') || lowerText.includes('check')) {
          botResponseText = "To track your application, please provide your **Application ID** (in the format `APP-2026-XXXX`). Enter it below and I'll fetch it instantly!";
          setIsWaitingForAppId(true);
        } else if (lowerText.includes('contact') || lowerText.includes('phone') || lowerText.includes('email') || lowerText.includes('call') || lowerText.includes('address') || lowerText.includes('location')) {
          botResponseText = "Reach the TMEC Admissions office at:\n* 📞 Phone: +91 98765 43210\n* ✉️ Email: admissions@tmec.edu.in\n* 📍 Campus: Thought Minds Engineering College, Bengaluru.\n\nYou can also leave an inquiry message on our [Contact Page](/contact).";
        } else if (lowerText.includes('hi') || lowerText.includes('hello') || lowerText.includes('hey') || lowerText.includes('greetings')) {
          botResponseText = "Hello! I am here to help you navigate TMEC Admissions. Ask me about our courses, eligibility criteria, semesters fees, or tracking status.";
        } else {
          botResponseText = "I apologize, I did not fully understand that query. I can help you with:\n\n* **Applying for Admissions**\n* **Tuition Fees & Eligibility**\n* **Engineering Branches & HOD details**\n* **Tracking Application Status (provide ID: APP-YYYY-XXXX)**\n* **Admissions Contact details**\n\nAlternatively, you can navigate using the quick reply buttons below.";
        }
      }
      ...
      {isOpen && (
        <div
          style={{
            ...
            borderRadius: '20px',
            boxShadow: 'var(--shadow-premium), 0 0 30px rgba(37, 99, 235, 0.1)',
            display: 'flex',
